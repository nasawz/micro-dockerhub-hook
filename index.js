...
module.exports = async (req, res) => {
  ...
  logger(
    ...
    `Payload from docker hub:\n ${JSON.stringify(
      ...
      null,
      2
    )} \nRunning hook on repo: ${payload.repository.repo_full_name}`
  );
  const hook = hooks(payload.repository.repo_full_name, payload.push_data.tag);

  try {
    const result = await runScript(hook, payload); // runs script
    logger(
      "debug",
      `${result}\nFinished running hook "${hook}" for repository "${
