...
module.exports = options => {
  ...
  } else if (!payload.repository.repo_full_name) {
    throw Error("Missing payload.repository.repo_full_name");
  } else if (!hooks(payload.repository.repo_full_name, payload.push_data.tag)) {
    throw Error(
      `${payload.repository.repo_full_name} does not exist in scripts/index.js`
    );
  }
};
